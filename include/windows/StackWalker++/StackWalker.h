...
// heavily shrinked by a_teammate (Inexor project)
//
...
/// Mods by inexor:
//  2015-12-09   v6b   - Removed anything for deprecated vs-versions and modify functionality to concat
//                       the callstack directly to a given in output string.
//  2015-29-11   v7    - Made the library header only
//  2015-01-12   v7b   - Output everything to a vector of std::string's (if passed to ShowCallstack(), otherwise printf) 
//                       and remove some more clutter.
//
///////////////////////////////////////////////////////////////////////////
...
#define CALLSTACK_STARTFRAME 3 // The first command we display is one of our crashreporting code, skip those
#define CALLSTACK_NUMFRAMES 10 // thats the number of callstack entries we display
...
public:
  typedef enum StackWalkOptions
  {
    ...
    RetrieveFileVersion = 8,// Also retrieve the version for the DLL/EXE, disabled
...
#pragma pack( pop )
    ...
    DWORD LoadModule(HANDLE hProcess, LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size)
    {
        ...
        if((m_parent != NULL) && (szImg != NULL))
        {
            ...
            this->m_parent->OnLoadModule(img, mod, baseAddr, size, result, szSymType, Module.LoadedImageName);
        }
...
// #############################################################
inline StackWalker::StackWalker(DWORD dwProcessId, HANDLE hProcess)
{
    output = NULL;
...
inline StackWalker::StackWalker(int options, LPCSTR szSymPath, DWORD dwProcessId, HANDLE hProcess)
{
    output = NULL;
...
inline StackWalker::~StackWalker()
{
    if(m_szSymPath != NULL)
        free(m_szSymPath);
    m_szSymPath = NULL;
...
inline BOOL StackWalker::LoadModules()
{
    if(this->m_sw == NULL)
    {
        SetLastError(ERROR_DLL_INIT_FAILED);
...
inline BOOL StackWalker::ShowCallstack(std::vector<std::string> *output, HANDLE hThread, const CONTEXT *context)
{
    this->output = output;
    CONTEXT c;;
...
#error "Platform not supported!"
    ...
    for(frameNum = 0; frameNum < (CALLSTACK_STARTFRAME + CALLSTACK_NUMFRAMES); ++frameNum)
    {
        ...
        if(frameNum < CALLSTACK_STARTFRAME) continue;
        ...
        if(s.AddrPC.Offset != 0)
        {
            ...
            { // yes, we have SymGetLineFromAddr64()
                if(this->m_sw->pSGLFA(this->m_hProcess, s.AddrPC.Offset, &(csEntry.offsetFromLine), &Line) != FALSE)
                {
                    ...
                    strcpy_s(csEntry.lineFileName, del ? del + 1 : Line.FileName);
                }
                else
                {
        ...
        if(frameNum == 0)
            et = firstEntry;
        this->OnCallstackEntry(et, csEntry, frameNum);

        if(s.AddrReturn.Offset == 0)
        {
            this->OnCallstackEntry(lastEntry, csEntry, frameNum);
...
inline BOOL __stdcall StackWalker::myReadProcMem(
    HANDLE      hProcess,
    DWORD64     qwBaseAddress,
    PVOID       lpBuffer,
    DWORD       nSize,
...
inline void StackWalker::OnLoadModule(LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size, DWORD result, LPCSTR symType, LPCSTR pdbName)
{
...
inline void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry, int frameNUM)
{
    CHAR buffer[STACKWALK_MAX_NAMELEN];
...
inline void StackWalker::OnDbgHelpErr(LPCSTR szFuncName, DWORD gle, DWORD64 addr)
{
    CHAR buffer[STACKWALK_MAX_NAMELEN];
    _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "Debug Help parsing ERROR: %s, GetLastError: %d (Address: %p)\n", szFuncName, gle, (LPVOID)addr);
    OnOutput(buffer);
}

inline void StackWalker::OnSymInit(LPCSTR szSearchPath, DWORD symOptions, LPCSTR szUserName)
{
    //CHAR buffer[STACKWALK_MAX_NAMELEN];
...
inline void StackWalker::OnOutput(LPCSTR buffer)
{
    if(output) output->push_back(buffer);
    else {
        printf(buffer);
